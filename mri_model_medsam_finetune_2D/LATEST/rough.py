...
def main():
    ...
    train_marksheet_csv = str(Path(args.histo_marksheet_dir) / "train.csv")
    val_marksheet_csv = str(Path(args.histo_marksheet_dir) / "val.csv")
    df_proxy_train = pd.read_csv(train_marksheet_csv)
    df_proxy_val = pd.read_csv(val_marksheet_csv)
    df_train = pd.concat([df_proxy_train, df_proxy_val], ignore_index=True)

    # -------- Histo buckets for triplet --------
    train_histo_buckets = get_histo_by_isup(
        encodings_dir=str(Path(args.histo_dir)),
    ...
    test_marksheet_csv = str(Path(args.histo_marksheet_dir) / "test.csv")
    df_test = pd.read_csv(test_marksheet_csv)
    ...
    for epoch in range(1, args.triplet_epochs + 1):
        ...
        if early_triplet.update(lr_metrics["macro_auc"], model, save_path=outdir / "ckpt_triplet_best_macro_auc.pt"):
            best_lr_clf = lr_metrics["clf"]
            print(f"  ↳ [triplet] new best (val macro-auc={early_triplet.best:.4f}) snapshot stored in memory")
        else:
